...
import { AlertCircle, Plus, Search, UserCheck, UserX, Eye, Trash2, Pencil } from "lucide-react";
...
export default function TupadApplicantsPage() {
  ...
  const [editingApplicant, setEditingApplicant] = useState<TupadApplicant | null>(null);
  const [form, setForm] = useState<TupadFormData>(initialFormData);
  const [formLoading, setFormLoading] = useState(false);
  
  // State for View Dialog
  const [isViewDialogOpen, setIsViewDialogOpen] = useState(false);
  const [selectedApplicant, setSelectedApplicant] = useState<TupadApplicant | null>(null);

  // State for Delete Dialog
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [applicantToDelete, setApplicantToDelete] = useState<TupadApplicant | null>(null);
  ...
  const handleSubmitForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || `Failed to ${editingApplicant ? "update" : "add"} applicant`);
      }

      setSuccess(`Applicant ${editingApplicant ? "updated" : "added"} successfully`);
      setIsFormDialogOpen(false);
      setForm(initialFormData);
      setEditingApplicant(null);
      fetchApplicants();
    } catch (err) {
      setError(err instanceof Error ? err.message : `Failed to ${editingApplicant ? "update" : "add"} applicant`);
    } finally {
            ...
            <div className="h-full w-full overflow-y-auto p-6 space-y-6">
              ...
              <form onSubmit={handleSubmitForm} className="space-y-8">
                ...
                <DialogFooter>
                  ...
                  >
                    {formLoading ? (
                      editingApplicant ? "Updating..." : "Saving..."
                    ) : (
                  ...
                  <TableBody>
                    {paginatedApplicants.map((a) => (
                      <TableRow key={a.id}>
                        ...
                        <TableCell>
                          {/* Format birthday as "January 01, 1991" */}
                          {a.birthday
                            ? new Date(a.birthday).toLocaleDateString("en-US", {
                                year: "numeric",
                                month: "long",
                                day: "2-digit",
                              })
          ...
          <div className="h-full overflow-y-auto p-8 space-y-8">
            <DialogHeader>
              ...
              <DialogDescription className="text-md text-gray-600">
                Full details for {selectedApplicant?.firstname} {selectedApplicant?.lastname}
              </DialogDescription>
            </DialogHeader>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium text-gray-800">
                        {selectedApplicant.firstname} {selectedApplicant.middlename} {selectedApplicant.lastname} {selectedApplicant.extension}
                      </p>
                    </div>
                    <div>
                      <Label className="text-gray-500">Birthday</Label>
                      <p className="font-medium text-gray-800">
                        {selectedApplicant.birthday ? new Date(selectedApplicant.birthday).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "2-digit" }) : 'N/A'}
                      </p>
                    </div>
                    <div>
                      <Label className="text-gray-500">Age</Label>
                      <p className="font-medium text-gray-800">{selectedApplicant.age}</p>
                    </div>
                    <div>
                      <Label className="text-gray-500">Sex</Label>
                      <p className="font-medium text-gray-800">{selectedApplicant.sex}</p>
                    </div>
                    <div>
                      <Label className="text-gray-500">Civil Status</Label>
                      <p className="font-medium text-gray-800">{selectedApplicant.civil_status}</p>
                    </div>
                    <div>
                      <Label className="text-gray-500">Contact Number</Label>
                      <p className="font-medium text-gray-800">{selectedApplicant.contact_number}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-b pb-4 border-red-100">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium text-gray-800">{selectedApplicant.barangay}</p>
                    </div>
                    <div>
                      <Label className="text-gray-500">City/Municipality</Label>
                      <p className="font-medium text-gray-800">{selectedApplicant.city_municipality}</p>
                    </div>
                    <div>
                      <Label className="text-gray-500">Province</Label>
                      <p className="font-medium text-gray-800">{selectedApplicant.province}</p>
                    </div>
                    <div>
                      <Label className="text-gray-500">District</Label>
                      <p className="font-medium text-gray-800">{selectedApplicant.district}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-b pb-4 border-red-100">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium text-gray-800">{selectedApplicant.type_of_id}</p>
                    </div>
                    <div>
                      <Label className="text-gray-500">ID Number</Label>
                      <p className="font-medium text-gray-800">{selectedApplicant.id_number}</p>
                    </div>
                    <div>
                      <Label className="text-gray-500">Bank Account No.</Label>
                      <p className="font-medium text-gray-800">{selectedApplicant.bank_account_no}</p>
                    </div>
                    <div>
                      <Label className="text-gray-500">Type of Beneficiary</Label>
                      <p className="font-medium text-gray-800">{selectedApplicant.type_of_beneficiary}</p>
                    </div>
                    <div>
                      <Label className="text-gray-500">Occupation</Label>
                      <p className="font-medium text-gray-800">{selectedApplicant.occupation}</p>
                    </div>
                    <div>
                      <Label className="text-gray-500">Monthly Income</Label>
                      <p className="font-medium text-gray-800">
                        {selectedApplicant.monthly_income
                          ? `Php ${Number(selectedApplicant.monthly_income).toLocaleString("en-PH", {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                            })}`
                          : "N/A"}
                    ...
                    <div>
                      ...
                      <p className="font-medium text-gray-800">{selectedApplicant.dependent_name}</p>
                    </div>
                    <div>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <span className="font-semibold">{applicantToDelete?.firstname} {applicantToDelete?.lastname}</span>?
